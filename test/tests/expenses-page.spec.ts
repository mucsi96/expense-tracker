...
test('chart tooltip shows only the hovered category and its total', async ({ page }) => {
  // 100.10 + 200.20 sums to 300.29999999999995 in floating point; the tooltip
  // must format it as 300.30
  ...
  // Snacks is by far the largest July category, so its segment tops the stack
  // and covers the center of the chart where hover() points
  await chartSection.getByRole('img', { name: /This is a chart/ }).hover();

  await expect(chartSection.locator('.chart-tooltip-name')).toHaveText('Snacks');
  await expect(chartSection.locator('.chart-tooltip-value')).toHaveText('300.30');
  // The other categories stay out of the tooltip
  await expect(chartSection.getByText('42.50', { exact: true })).toHaveCount(0);
  await expect(chartSection.getByText('12.80', { exact: true })).toHaveCount(0);
});
